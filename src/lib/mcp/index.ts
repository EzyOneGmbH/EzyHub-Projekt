...
export default defineMcp({
  ...
  version: "0.1.0",
  instructions:
    "Read-only access to EZY ONE / EzyHub: list Kunden (clients) and content items (Blogs, Landingpages, GEO, Erfolge, Reports, Notes) scoped to the signed-in user's organization via Supabase RLS. Use `whoami` to verify the connection.",
  auth: auth.oauth.issuer({
    issuer: `https://${projectRef}.supabase.co/auth/v1`,
    acceptedAudiences: "authenticated",
  }),
  tools: [whoamiTool, listClientsTool, listContentItemsTool],
});
