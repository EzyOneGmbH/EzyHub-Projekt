...
export function friendlyWpError(msg) {
  const m = String(msg || "");
  if (/401|403|unauthorized|forbidden|incorrect_password|invalid_username|application password/i.test(m))
 ...
 */
export default function WordPressPublishModal({ clientId, defaultTitle, markdown, onClose, notify, zIndex = 120 }) {
  ...
  const publish = async () => {
    ...
    try {
      ...
      if (j.ok) {
        ...
        if (j.seoPluginMissing) notify?.("Kein SEO-Plugin (Yoast/RankMath) erkannt — SEO-Felder übersprungen", "info");
      } else {
  ...
  return (
    ...
    >
      <div style={{ background: C.surface, borderRadius: 16, width: "100%", maxWidth: 560, maxHeight: "90vh", overflow: "auto", boxShadow: "0 20px 60px rgba(0,0,0,.5)" }}>
          ...
          {status === "ready" && !result && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ fontSize: 12, color: C.textDim, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                <span>Ziel: <span style={{ color: C.text }}>{siteUrl}</span></span>
                <button
                  onClick={() => setShowPreview((v) => !v)}
                  style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "none", border: `1px solid ${C.border}`, borderRadius: 8, padding: "3px 10px", cursor: "pointer", color: C.accent, fontSize: 11.5, fontWeight: 600, fontFamily: "inherit" }}
                >
              ...
              {showPreview && (
                <div
                  className="ezy-md"
                  style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: 14, maxHeight: 260, overflow: "auto" }}
              ...
              {wpStatus === "publish" && (
                <label style={{ display: "flex", gap: 8, alignItems: "flex-start", fontSize: 12.5, color: C.text, background: `${C.orange}14`, border: `1px solid ${C.orange}55`, borderRadius: 10, padding: "10px 12px", cursor: "pointer" }}>
                  <input
                    type="checkbox"
                  ...
                  />
                  <span>
                    <AlertTriangle size={13} style={{ verticalAlign: "-2px", marginRight: 4 }} color={C.orange} />
                    Der Beitrag geht damit <strong>sofort öffentlich live</strong> auf {siteUrl || "der Website"}. Ich bin sicher.
                  </span>
                </label>
              )}
              <div>
              ...
              {error && (
                <div style={{ fontSize: 12.5, color: "#dc2626", background: "#dc262614", border: "1px solid #dc262655", borderRadius: 10, padding: "10px 12px", lineHeight: 1.5 }}>
                  {error}
                </div>
              )}
            </div>
        ...
        <div style={{ padding: "16px 24px", borderTop: `1px solid ${C.border}`, display: "flex", justifyContent: "flex-end", gap: 10, alignItems: "center" }}>
          {result ? (
            <button onClick={onClose} style={{ background: "linear-gradient(135deg,#71008B,#B9009C)", color: "#fff", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
              Schliessen
            </button>
          ) : (
            <>
              <button onClick={onClose} style={{ background: C.card, color: C.text, border: `1px solid ${C.border}`, borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
                Abbrechen
              </button>
              {status === "ready" && (
                <button
                  ...
                  title={needsConfirm ? "Bestätige zuerst den Haken für sofortiges Veröffentlichen" : undefined}
                  style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "linear-gradient(135deg,#71008B,#B9009C)", color: "#fff", border: "none", borderRadius: 8, padding: "7px 14px", fontSize: 12, fontWeight: 600, cursor: busy || !title.trim() || needsConfirm ? "default" : "pointer", opacity: busy || !title.trim() || needsConfirm ? 0.5 : 1, fontFamily: "inherit" }}
                >
                  <Globe size={14} />
