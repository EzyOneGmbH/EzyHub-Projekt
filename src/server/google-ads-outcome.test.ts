...
describe("computeVerdict (4.1)", () => {
  it("positive bei CPA-Verbesserung > 5%", () => {
    expect(computeVerdict({ cost: 500, conversions: 10 }, { cost: 450, conversions: 10 })).toBe("positive"); // 50 -> 45 = -10%
  });
  it("negative bei Verschlechterung > 10%", () => {
    expect(computeVerdict({ cost: 500, conversions: 10 }, { cost: 600, conversions: 10 })).toBe("negative"); // 50 -> 60 = +20%
  });
  it("neutral in der Totzone (-5%..+10%)", () => {
    expect(computeVerdict({ cost: 500, conversions: 10 }, { cost: 520, conversions: 10 })).toBe("neutral"); // +4%
    expect(computeVerdict({ cost: 500, conversions: 10 }, { cost: 490, conversions: 10 })).toBe("neutral"); // -2%
  });
  it("inconclusive bei zu wenig Daten", () => {
    expect(computeVerdict({ cost: 100, conversions: 1 }, { cost: 100, conversions: 2 })).toBe("inconclusive"); // < 5 Conv. gesamt
    expect(computeVerdict({ cost: 0, conversions: 0 }, { cost: 100, conversions: 10 })).toBe("inconclusive"); // kein Vorher-Spend
    expect(computeVerdict({ cost: 100, conversions: 8 }, { cost: 100, conversions: 0 })).toBe("inconclusive"); // CPA nachher undefiniert
  });
});

const row = (over: Partial<ScoreRow>): ScoreRow => ({
  action_type: "add_negative", n_total: 0, n_positive: 0, n_neutral: 0,
  n_negative: 0, n_inconclusive: 0, hit_rate: null, avoided_waste_chf_sum: 0, ...over,
});

describe("checkAutonomy (4.3 Freischaltkriterien)", () => {
  it("Level 0->1 erst ab n>=15, hit>=90%, 0 negative", () => {
    const ok = checkAutonomy([row({ n_total: 16, n_positive: 12, n_neutral: 3, n_inconclusive: 1 })], false);
    ...
    const hasNegative = checkAutonomy([row({ n_total: 20, n_positive: 18, n_neutral: 1, n_negative: 1 })], false);
  ...
  it("Level 1->2 nur mit bid_writes_enabled UND Bid-Scorecard", () => {
    const rows = [row({ action_type: "bid_adjustment", n_total: 12, n_positive: 8, n_neutral: 2, n_inconclusive: 2 })];
...
describe("Parsing-Helfer", () => {
  ...
  it("extrahiert Negative-Term aus after_value", () => {
    expect(termFromAfterValue('+ "billige unterkunft" (negative exact)')).toBe("billige unterkunft");
