...
export function checkAutonomy(
  ...
  for (const r of rows) {
    if (r.n_total >= 4 && r.n_inconclusive / r.n_total > 0.5)
      dataProblems.push(
        `${r.action_type}: ${r.n_inconclusive}/${r.n_total} inconclusive (>50%) - Datenproblem statt weiterer Empfehlungen melden`,
      );
  }
  // Level 0 -> 1 (Negatives): n_total >= 15, hit_rate >= 90%, n_negative == 0 (90d)
  const negRows = rows.filter(
    (r) => r.action_type === "add_negative" || r.action_type === "add_negative_phrase",
  );
  ...
  const negativesReadyForL1 =
    negTotal >= 15 && negNegative === 0 && negHitRate != null && negHitRate >= 0.9;
  reasons.push(
  ...
  const bidsReadyForL2 =
    !!bid && bidWritesEnabled && bid.n_total >= 10 && bidHitRate != null && bidHitRate >= 0.8;
  reasons.push(
    ...
    return {
      ...
      headers: {
        ...
        ...(process.env.GOOGLE_ADS_LOGIN_CUSTOMER_ID
          ? {
              "login-customer-id": String(process.env.GOOGLE_ADS_LOGIN_CUSTOMER_ID).replace(
                /\D/g,
                "",
              ),
            }
...
async function campaignMetrics(
  ...
  for (const r of rows) {
    ...
    if (r.metrics?.searchImpressionShare != null) {
      isSum += Number(r.metrics.searchImpressionShare);
      isN += 1;
    }
  }
  return {
...
export async function runOutcomeReview(clientId?: string): Promise<{
  ...
  const out = {
    ok: true,
    reviewed: 0,
    downgrades: [] as Array<{ clientId: string; from: number; to: number; reason: string }>,
    errors: [] as string[],
  };
  const now = new Date();
  ...
  for (const ch of changes) {
    ...
    try {
      ...
      if (!ctxCache.has(ch.client_id))
        ctxCache.set(
          ch.client_id,
          await ctxFor(ch.client_id, clientRows.get(ch.client_id)?.google_ads_customer),
        );
      const ctx = ctxCache.get(ch.client_id);
      if (!ctx) {
        out.errors.push(`${ch.id}: kein Ads-Kontext`);
        continue;
      }

      const campaign = campaignFromEntity(ch.entity);
      if (!campaign) {
        out.errors.push(`${ch.id}: keine Kampagne im Entity`);
        continue;
      }
      ...
      for (const win of windows) {
        ...
        if (
          ch.action_type.startsWith("add_negative") ||
          ch.action_type === "negative_keyword_semantic"
        ) {
          ...
          if (term) {
            try {
              ...
              avoided =
                Math.round(
                  rows.reduce((s, r) => s + Number(r.metrics?.costMicros ?? 0) / MICROS, 0) * 100,
                ) / 100;
            } catch {
              /* Phrase-Muster nicht exakt abfragbar -> null */
            }
          }
        }
  ...
  for (const cid of affectedClients) {
    ...
    if (cur >= 1) {
      ...
      await supabaseAdmin
        .from("ads_autopilot_config")
        .update({ autonomy_level: to })
        .eq("client_id", cid);
      await supabaseAdmin.from("ads_changelog").insert({
        client_id: cid,
        customer_id: "-",
        run_id: `${isoDay(now)}-downgrade`,
        action_type: "autonomy_downgrade",
        action_class: "report-only",
        entity: "Konto",
        before_value: `autonomy_level ${cur}`,
        after_value: `autonomy_level ${to}`,
        rationale: `Automatische Herabstufung: ${reason}`,
        status: "executed",
      });
