...
describe("activeSeasonWindow (Abnahme 1.5)", () => {
  it("findet Hochsaison-Fenster inkl. Jahreswechsel", () => {
    ...
    expect(activeSeasonWindow(c, new Date("2026-12-25"))).toEqual({
      kind: "high",
      window: "12-20..01-06",
    });
    expect(activeSeasonWindow(c, new Date("2027-01-03"))).toEqual({
      kind: "high",
      window: "12-20..01-06",
    });
...
describe("planActions (Abnahme 1.4)", () => {
  ...
  it("Cross-Kampagnen-Konflikt: konvertierender Begriff wird NICHT ausgeschlossen", () => {
    ...
    const acts = planActions(d, cfg());
    expect(
      acts.some((a) => a.type === "add_negative" && a.rationale.includes("wellness wochenende")),
    ).toBe(false);
      ...
      campaigns: [
        {
          ...
          roas: 5,
          budgetLostIs: 0,
          biddingSystemStatus: "LEARNING_BUDGET_CHANGE",
          learning: true,
        },
      ],
      searchTerms: [
        {
          term: "teurer quatsch begriff",
          campaign: "L",
          adGroup: "AG",
          costChf: 200,
          conversions: 0,
  ...
  it("target_roas gesetzt -> ROAS-Pruefung hat Vorrang (CPA irrelevant)", () => {
    ...
    expect(
      planActions(d, cfg({ target_roas: 4, target_cpa_chf: 50 })).filter(
        (a) => a.type === "budget_change",
      ),
    ).toHaveLength(0);
    // ROAS 2.0 >= Ziel 1.5 -> Vorschlag mit ROAS-Beleg
    const ok = planActions(d, cfg({ target_roas: 1.5, target_cpa_chf: 50 })).filter(
      (a) => a.type === "budget_change",
    );
...
describe("computeGeoDeviceFindings (Phase 3.2)", () => {
  ...
  it("meldet Geo-Kostenfresser ohne Conversions (> 3x CPA)", () => {
    ...
    expect(
      computeGeoDeviceFindings(d).some(
        (x) => x.entity === "USA" && /Streuverlust/.test(x.rationale),
      ),
    ).toBe(true);
  });
...
describe("computeStructureFindings (Struktur-Review, report-only)", () => {
  ...
  it("liefert NIE etwas anderes als report-only", () => {
    const d = data({
      ...
      adGroupPerformance: [
        { campaign: "K", adGroup: "A", costChf: 900, conversions: 0, convValue: 0 },
      ],
      monthComparison: [
        {
          campaign: "K",
      ...
      campaigns: [
        {
          ...
          searchAbsTopImpressionShare: 0.5,
          rankLostIs: 0.08,
        },
        {
          name: "SN - DE - Brand - OK",
          ...
          searchAbsTopImpressionShare: 0.9,
        },
        {
          name: "Non-Brand Y",
          status: "ENABLED",
          ...
          searchAbsTopImpressionShare: 0.1,
        },
      ],
    });
    const f = computeBrandOtaFindings(d, cfg({ industry: "hotel" })).filter(
      (x) => x.type === "brand_is_alert",
    );
    expect(f).toHaveLength(1);
  ...
  it("quantifiziert OTA-Druck auf Brand-Kampagnen (nur industry hotel)", () => {
    const d = data({
      auctionInsights: [
        ...
        {
          campaign: "SN - DE - Brand - X",
          domain: "eigenerandererhotelier.ch",
          impressionShare: 0.1,
          overlapRate: 0.2,
          outrankingShare: 0.05,
        },
        {
          campaign: "Non-Brand",
          domain: "booking.com",
