    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const { data: existing } = await supabaseAdmin
          ...
          .maybeSingle();
        if (existing)
          return Response.json({ ok: true, deduped: true, approvalId: existing.id });
        ...
        const { data: logRow } = await supabaseAdmin
          ...
          .insert({
            ...
            action_type: "campaign_proposal", action_class: "approval-needed",
            entity: title, before_value: "", after_value: docPath,
            rationale: summary ?? `Kampagnen-Proposal: ${title}`, status: "pending",
          })
        ...
        const { data: appr, error } = await supabaseAdmin
          .from("ads_approvals")
          .insert({
            client_id: clientId, customer_id: customerId || "-", run_id: runId, action_id: "p-001",
            type: "campaign_proposal", entity: title,
            current_value: "", proposed_value: docPath,
            rationale: summary ?? null, estimated_impact: estimatedImpact ?? null,
