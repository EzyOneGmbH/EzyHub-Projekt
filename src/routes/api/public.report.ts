    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const { error: insErr } = await (supabaseAdmin as any).from("public_report_links").insert({
          ...
          client_id: clientId,
          token_hash: reportTokenHash(token),
      ...
      GET: async ({ request }) => {
        ...
        const { data: rep } = await sb
          ...
          .maybeSingle();
        if (!rep)
          return Response.json({ ok: false, error: "Noch kein Report vorhanden" }, { status: 404 });
        const [models, topics, sources, history, sov] = await Promise.all([
          ...
          sb
            ...
            .select("domain, mentions, share, urls")
            .eq("report_id", rep.id)
            .order("mentions", { ascending: false })
            .limit(30),
          sb
            .from("ai_visibility_reports")
            .select("snapshot_date, score, mentions, citations, cited_pages")
            .eq("client_id", clientId)
            .order("snapshot_date", { ascending: true })
            .limit(400),
          sb
            .from("ai_visibility_sov")
            .select("brand, is_self, mentions, share")
            .eq("report_id", rep.id)
        ...
        return Response.json(
          {
            ...
            date: rep.snapshot_date,
            score: rep.score,
            scoreDelta: rep.score_delta,
            mentions: rep.mentions,
            citations: rep.citations,
            citedPages: rep.cited_pages,
      ...
      DELETE: async ({ request }) => {
        ...
        if (/^[0-9a-f-]{36}$/i.test(linkId)) q = q.eq("id", linkId);
          ...
          return Response.json(
            { ok: false, error: "linkId oder clientId erforderlich" },
            { status: 400 },
          );
