...
export type BotDetail = {
  name: string;
  owner: string;
  status: "erlaubt" | "blockiert" | "unspezifiziert";
  eigeneRegel: boolean;
};
export function botDetailsFor(robotsTxt: string): BotDetail[] {
  return AI_BOT_DEFS.map((b) => {
    ...
    return {
      name: b.name,
      owner: b.owner,
      status: blocked ? "blockiert" : eigeneRegel ? "erlaubt" : "unspezifiziert",
      eigeneRegel,
    };
  });
}
...
export async function fetchText(url: string, timeoutMs = 15000): Promise<Fetched> {
  ...
  try {
    const r = await fetch(url, {
      redirect: "follow",
      headers: {
        "User-Agent": "Mozilla/5.0 (compatible; EzyHubSiteHealth/1.0; +https://ezyhub.ch)",
      },
...
function analyzeHtml(html: string, domain: string) {
  ...
  return {
    ...
    internalLinks: (
      html.match(
        new RegExp(
          `<a[^>]+href=["'](?:https?://(?:www\\.)?${domain.replace(/\./g, "\\.")})?/[^"']*["']`,
          "gi",
        ),
      ) || []
    ).length,
...
const PAGE_CHECKS: Array<{
  ...
  severity: Severity;
  tipp: string;
  score: (m: PageMetrics) => number;
  detail: (m: PageMetrics) => string;
}> = [
  {
    id: "title",
    label: "Seitentitel gesetzt und sinnvoll lang",
    pillar: "content",
    weight: 3,
    severity: "hoch",
  ...
  {
    id: "metadesc",
    label: "Meta-Description gesetzt (50–160 Zeichen)",
    pillar: "content",
    weight: 2,
    severity: "mittel",
  ...
  {
    id: "structure",
    label: "Inhalts-Struktur (H2-Zwischentitel)",
    pillar: "content",
    weight: 1,
    severity: "niedrig",
  ...
  {
    id: "wordcount",
    label: "Ausreichender Textumfang",
    pillar: "content",
    weight: 2,
    severity: "mittel",
  ...
  },
  {
    ...
    score: (m) => (m.canonical ? 1 : 0),
    detail: (m) => (m.canonical ? "vorhanden" : "fehlt"),
  },
  {
    id: "schema",
  ...
  {
    id: "faq",
    label: "Antwort-Format (FAQ/Frage-Abschnitte)",
    pillar: "aeo",
    weight: 2,
    severity: "mittel",
    tipp: "FAQ-Abschnitte mit echten Fragen als Überschrift (+ FAQPage-Schema).",
    score: (m) =>
      [...m.ldTypes].some((t) => /faqpage|howto|question/i.test(t)) || m.questionHeadings > 0
        ? 1
        : m.lists > 0
          ? 0.5
          : 0,
    detail: (m) =>
      [...m.ldTypes].some((t) => /faqpage|howto/i.test(t))
        ? "FAQ/HowTo-Schema"
        : m.questionHeadings
          ? `${m.questionHeadings} Frage-Überschriften`
          : m.lists
            ? "nur Listen"
            : "keine",
  },
...
export async function runSiteHealthForDomain(domain: string, mode: "quick" | "deep") {
  ...
  for (const u of pageUrls.slice(1)) {
    if (Date.now() - started > TIME_BUDGET_MS) {
      truncated = pageUrls.length - 1 - (pageResults.length - 1);
      break;
    }
    await pause(350);
  ...
  const push = (
    c: Omit<CheckOut, "status" | "detail"> & {
      status: CheckOut["status"];
      detail: string;
      pagesAffected?: string[];
    },
  ) => checks.push(c as CheckOut);
  ...
  push({
    ...
    status: home.ok ? "ok" : "fail",
    detail: home.ok
      ? `HTTP ${home.status} in ${home.ms} ms`
      : `Startseite nicht ladbar (HTTP ${home.status})`,
  });
  push({
    id: "speed",
    label: "Antwortzeit der Startseite",
    pillar: "technical",
    weight: 2,
    severity: "hoch",
    tipp: "Server-Antwortzeit über Caching/Hosting verbessern — langsame Seiten lesen KI-Crawler seltener vollständig.",
    status: home.ms < 1500 ? "ok" : home.ms < 4000 ? "warn" : "fail",
    detail: `${home.ms} ms bis zur Antwort`,
  });
  push({
    id: "robots",
    ...
    status: robots.ok ? "ok" : "fail",
    detail: robots.ok ? "vorhanden" : `fehlt (HTTP ${robots.status})`,
  });
  const sitemapInRobots = /^sitemap:/im.test(robots.text || "");
  push({
    id: "sitemap",
    label: "XML-Sitemap auffindbar",
    pillar: "technical",
    weight: 2,
    severity: "mittel",
    ...
    detail: sitemap.ok
      ...
      : sitemapInRobots
        ...
        : "weder /sitemap.xml noch robots-Eintrag",
  });
  push({
    id: "viewport",
    label: "Mobile Viewport konfiguriert",
    pillar: "technical",
    weight: 2,
    severity: "hoch",
    tipp: "<meta name=viewport> ergänzen — ohne sie ist die Seite mobil kaum nutzbar.",
    status: homeMetrics?.viewport ? "ok" : "fail",
    detail: homeMetrics?.viewport ? "vorhanden" : "fehlt",
  });
  push({
    id: "lang",
    label: "Sprache im HTML deklariert",
    pillar: "technical",
    weight: 1,
    severity: "niedrig",
    tipp: "<html lang=…> setzen — hilft Suche und Screenreadern.",
    status: homeMetrics?.langAttr ? "ok" : "fail",
    detail: homeMetrics?.langAttr ? "vorhanden" : "fehlt",
  });
  if (mode === "deep" && truncated > 0) {
    // Gewicht 0: reine Info, verfälscht keinen Score.
    push({
      id: "coverage",
      label: "Seiten-Abdeckung des Tiefen-Audits",
      pillar: "technical",
      weight: 0,
      severity: "niedrig",
      tipp: "Der Hoster antwortete langsam — der Audit stoppte am Zeitbudget. Erneut ausführen prüft wieder von vorn.",
      status: "warn",
      detail: `${pageResults.length} geprüft, ${truncated} übersprungen (Zeitbudget)`,
    });
  }
  if (mode === "deep") {
    ...
    push({
      id: "pagesok",
      label: "Alle geprüften Seiten erreichbar",
      pillar: "technical",
      weight: 2,
      severity: "hoch",
  ...
  for (const def of PAGE_CHECKS) {
    ...
    const detail =
      okPages.length === 1 && okPages[0].metrics
    ...
    push({
      ...
      status,
      detail,
      ...(affected.length && okPages.length > 1 ? { pagesAffected: affected.slice(0, 10) } : {}),
    });
  }
  ...
  push({
    id: "aibots",
    label: "KI-Crawler in robots.txt erlaubt",
    pillar: "aeo",
    weight: 3,
    severity: "kritisch",
  ...
  push({
    id: "orgschema",
    label: "Organisations-/LocalBusiness-Schema",
    pillar: "aeo",
    weight: 2,
    severity: "mittel",
    tipp: "Organization- oder LocalBusiness-Markup mit Name, Logo und Adresse hinterlegen.",
    status: [...allLdTypes].some((t) =>
      /organization|localbusiness|hotel|store|restaurant/i.test(t),
    )
      ? "ok"
      : "fail",
    detail: [...allLdTypes].some((t) =>
      /organization|localbusiness|hotel|store|restaurant/i.test(t),
    )
      ? "vorhanden"
      : "fehlt",
  ...
  push({
    ...
    status: llms.ok ? "ok" : "warn",
    detail: llms.ok ? "vorhanden" : "fehlt (optional)",
  });
  ...
  const issues = checks
    ...
    .map((c) => ({
      ...
      detail: c.detail,
      tipp: c.tipp,
      ...(c.pagesAffected?.length ? { pages: c.pagesAffected } : {}),
    }))
    .sort(
      (a, b) =>
        ["kritisch", "hoch", "mittel", "niedrig"].indexOf(a.severity) -
        ["kritisch", "hoch", "mittel", "niedrig"].indexOf(b.severity),
    );
  ...
  const pages = pageResults.map((p) => {
    ...
    return {
      ...
      title: p.metrics.title.slice(0, 80),
      issues: bad,
    };
  });
  ...
  return {
    url: base,
    mode,
    scores: { overall, technical, content, aeo },
    checks,
    issues,
    pages,
    blockedBots,
    ...
    homeHtml: home.text,
    robotsOk: robots.ok,
    botDetails: robots.ok ? botDetailsFor(robots.text) : [],
  };
}
