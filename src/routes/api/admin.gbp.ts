...
import { gbpResolveDefault, gbpReviews, gbpReplyReview, gbpCreatePost, gbpAccounts, gbpLocations } from "@/server/gbp.server";
      ...
      POST: async ({ request }) => {
        ...
        try {
          ...
          if (d.action === "discover") {
            ...
            if (!("accounts" in acc) || !acc.ok) return Response.json({ ok: false, error: (acc as any).error || "Keine Accounts", hint: "Business Profile API freigeschaltet? Google neu verbinden (Scope business.manage)?" });
            const account = acc.accounts[0]?.name;
            const locs = account ? await gbpLocations(clientId, account) : { ok: false, locations: [] as any[] };
            return Response.json({ ok: true, accounts: acc.accounts, locations: (locs as any).locations || [] });
          }
          ...
          if (!account || !location) {
            ...
            if (!def.ok) return Response.json({ ok: false, error: def.error, hint: "Business Profile API freigeschaltet? Google mit Scope business.manage neu verbinden?" });
            account = def.account; location = def.location;
