...
import { GA4_ADMIN_API, parseAccountSummaries } from "@/server/ga4.server";
...
// 21.09.2026: je Property zusaetzlich canEdit (Admin API PropertySummary.can_edit,
// neu 18.06.2026) — sagt UI/Readiness, ob wir Custom Dimensions/Key Events
// anlegen duerfen. null = Feld vom Server nicht geliefert.
    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          const r = await fetch(`${GA4_ADMIN_API}/accountSummaries?pageSize=200`, {
          ...
          const properties = parseAccountSummaries(await r.json());
