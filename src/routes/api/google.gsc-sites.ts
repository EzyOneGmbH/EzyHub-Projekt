          ...
          if (!r.ok) {
            ...
            if (API_DISABLED.test(t))
              return Response.json({
                ok: false,
                error: "Search Console API nicht aktiviert ('APIs & Services → Library' aktivieren).",
              });
            return Response.json({ ok: false, error: redactSecrets(`GSC sites.list HTTP ${r.status}: ${t}`) });
          }
