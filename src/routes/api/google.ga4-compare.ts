      ...
      POST: async ({ request }) => {
        try {
          ...
          const res = await fetch(url, {
            ...
            body: JSON.stringify({
              dateRanges: [
                { startDate: isoDay(parsed.data.start), endDate: isoDay(parsed.data.end), name: "current" },
                { startDate: isoDay(parsed.data.compareStart), endDate: isoDay(parsed.data.compareEnd), name: "compare" },
              ],
          ...
          const currentRow = rows.find((r) => /current|date_range_0/i.test(r.dimensionValues?.[0]?.value ?? "")) ?? rows[0];
          const compareRow = rows.find((r) => /compare|date_range_1/i.test(r.dimensionValues?.[0]?.value ?? "")) ?? rows[1];
