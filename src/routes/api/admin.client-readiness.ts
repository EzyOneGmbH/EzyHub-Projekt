...
async function requireOwnerAdmin(request: Request): Promise<{ userId: string; organizationId: string } | Response> {
...
const RUN_TYPES = [
  "populate_meta", "rankings", "gsc_summary",
  "canonry_ai_visibility", "llm_responses", "ai_citations",
  "google_ads",
];

async function buildSnapshot(organizationId: string, clientId: string): Promise<{ snapshot: ReadinessSnapshot; client: any } | null> {
  const { data: client } = await SB
    .from("clients")
    .select("id, name, domain, gsc_property, ga4_property, canonry_project, google_ads_customer, metadata")
    .eq("id", clientId).eq("organization_id", organizationId).maybeSingle();
  if (!client) return null;

  const [caa, integ, oauth, viewer, runs] = await Promise.all([
    SB.from("client_app_access").select("app, enabled, features")
      .eq("organization_id", organizationId).eq("client_id", clientId),
    SB.from("client_integrations").select("provider, enabled")
      .eq("organization_id", organizationId).eq("client_id", clientId),
    SB.from("oauth_connections").select("provider, client_id")
      .eq("organization_id", organizationId).or(`client_id.eq.${clientId},client_id.is.null`),
    SB.from("client_access").select("user_id, app_users!inner(role)")
      .eq("organization_id", organizationId).eq("client_id", clientId)
      .eq("app_users.role", "viewer"),
    SB.from("audit_runs").select("audit_type, finished_at, created_at")
      .eq("organization_id", organizationId).eq("client_id", clientId)
      .eq("status", "succeeded").in("audit_type", RUN_TYPES)
  ...
  const snapshot: ReadinessSnapshot = {
    ...
    oauth: {
      ...
      wordpress: (oauth.data ?? []).some((o: any) => o.provider === "wordpress" && o.client_id === clientId),
    },
    felder: {
      gsc_property: client.gsc_property, ga4_property: client.ga4_property,
      canonry_project: client.canonry_project, google_ads_customer: client.google_ads_customer,
    },
    lastRuns,
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (!res) return Response.json({ ok: false, error: "Kunde nicht gefunden" }, { status: 404 });
        return Response.json({ ok: true, snapshot: res.snapshot, readiness: evaluateReadiness(res.snapshot) });
      },
      ...
      POST: async ({ request }) => {
        ...
        const { data: own } = await SB.from("clients").select("id")
          .eq("id", clientId).eq("organization_id", u.organizationId).maybeSingle();
        if (!own) return Response.json({ ok: false, error: "Kunde nicht gefunden" }, { status: 404 });

        if (body.action === "datenlauf") {
          const secret = process.env.ADMIN_AUTOMATION_SECRET;
          if (!secret) return Response.json({ ok: false, error: "Automation nicht konfiguriert" }, { status: 503 });
          ...
          try {
            ...
            return Response.json({ ok: false, error: `Datenlauf: ${e instanceof Error ? e.message : String(e)}` }, { status: 502 });
          }
        }
