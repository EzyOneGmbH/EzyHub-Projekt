    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const questions: string[] = (Array.isArray(src.questions) ? src.questions : [])
          .map((q: any) => String(q?.q ?? q).trim()).filter(Boolean).slice(0, 8);
        const rival = String(src.rival || "").slice(0, 120);
        const thema = String(src.thema || questions[0] || "").slice(0, 200);
        if (!thema) return Response.json({ ok: false, error: "source.thema oder source.questions erforderlich" }, { status: 400 });
        ...
        const { data: rep } = await sbA
          .from("ai_visibility_reports").select("parts").eq("client_id", clientId)
          .order("snapshot_date", { ascending: false }).limit(1).maybeSingle();
        const fanout: string[] = ((rep?.parts?.sa?.chatgptFanout as any[]) || [])
          .flatMap((f: any) => f?.queries || []).map((q: any) => String(q)).slice(0, 15);
        ...
        const txt = await askUtility(
          ...
          `Thema/Anlass: ${thema}\n` +
          (rival ? `Kontext: Der Konkurrent "${rival}" wird bei diesen echten Nutzerfragen zitiert, ${client.name} nicht:\n${questions.map((q) => `- ${q}`).join("\n")}\n` : "") +
          (fanout.length ? `Echte KI-Folgefragen aus Messläufen (als Gliederungs-Kandidaten):\n${fanout.map((q) => `- ${q}`).join("\n")}\n` : "") +
        ...
        if (!txt || String(txt).trim().length < 100)
          return Response.json({ ok: false, error: "Brief-Generierung fehlgeschlagen (Utility-LLM)" }, { status: 502 });

        const briefMd = String(txt).trim().slice(0, 20000);
        const title = (briefMd.match(/^#\s*(.+)$/m)?.[1] || thema).slice(0, 200);
        const { data: ins, error } = await sbA.from("content_briefs").insert({
          organization_id: client.organization_id, client_id: clientId,
          title, brief_md: briefMd,
