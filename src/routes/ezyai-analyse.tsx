...
const STAGES: Array<{ keys: string[]; name: string; desc: string }> = [
  ...
  },
  {
    keys: ["seo"],
    name: "SEO-Fundament",
    desc: "Sichtbarkeit, Traffic-Schaetzung, Backlinks, LLM-Mentions-Makro",
  },
  { keys: ["volumen"], name: "AI-Suchvolumen", desc: "Monatliche AI-Nachfrage je Prompt" },
  {
    keys: ["ai1", "ai2", "ai3"],
    name: "AI-Sichtbarkeit",
    desc: "15 Prompts (max. 3 Brand) über alle 6 Engines inkl. Grok & DeepSeek",
  },
  {
    keys: ["entitaet"],
    name: "Entität & Vertrauen",
    desc: "Wikidata, Organization-Schema, Brand-SERP",
  },
  { keys: ["benchmark"], name: "Wettbewerber-Benchmark", desc: "Quick-Score je Wettbewerber" },
  {
    keys: ["score"],
    name: "Scoring & Massnahmen",
...
function Sev({ v }: { v: string }) {
  const map: Record<string, { bg: string; fg: string }> = {
    kritisch: { bg: S.redDim, fg: S.red },
    hoch: { bg: S.redDim, fg: S.red },
    mittel: { bg: S.orangeDim, fg: S.orange },
    niedrig: { bg: S.greenDim, fg: S.green },
...
function EzyAiAnalyseApp() {
  ...
  useEffect(() => {
    if (!authLoading && !session)
      navigate({ to: "/login", search: { next: "/ezyai-analyse" }, replace: true });
  ...
  useEffect(() => {
    if (session && role && role !== "viewer") void loadList();
  }, [session, role, loadList]);
              ...
              prefill={
                detail
                  ? {
                      ...
                      firmenname: detail.firmenname,
                      branche: detail.branche || "",
                      ort: detail.ort || "",
                    }
                  : null
              }
    ...
    <>
      <div>
        ...
        <div style={{ fontSize: 13, color: S.mut, marginTop: 4 }}>
          Pre-Check für Leads & Pitches — immer im vollen Umfang: alle Engines, AI-Suchvolumen,
          SiteHealth, Wettbewerber-Benchmark.
        </div>
      </div>
        ...
        {(["analyse", "verlauf"] as const).map((t) => (
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              fontFamily: "inherit",
              border: `1px solid ${tab === t ? S.app : S.line}`,
              background: tab === t ? S.tint : "transparent",
              color: tab === t ? S.app : S.mut,
            }}
          >
      ...
      {tab === "analyse" ? (
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 240 }}>
            ...
            >
              Wie sichtbar ist dein Lead in AI-Suchen?
            </h2>
            <p style={{ margin: 0, fontSize: 13.5, color: S.mut, maxWidth: "52ch" }}>
              Ein Lauf, volles Programm: AI-Sichtbarkeit über alle Engines, verpasstes
              AI-Suchvolumen, SiteHealth, Entität, Anbindungs-Check und Wettbewerber-Benchmark —
              fertig in ~15 Minuten.
            </p>
          </div>
          <button style={ctaBtn} onClick={onNew}>
            ＋ Neue Analyse
          </button>
        </div>
      ) : (
        <div style={card}>
          ...
          <p style={{ fontSize: 12.5, color: S.mut, margin: "0 0 14px" }}>
            Alle Analysen der Organisation — Re-Run jederzeit, z. B. fürs Follow-up nach 3 Monaten.
          </p>
            ...
            >
              <thead>
                <tr>
                  ...
                  <th style={{ ...th, textAlign: "right" }}>Verpasstes AI-Vol./Mt.</th>
                  <th style={th}></th>
                </tr>
              </thead>
              <tbody>
                ...
                {list !== null && list.length === 0 && (
                  <tr>
                    <td style={{ ...td, textAlign: "center", color: S.dim }} colSpan={7}>
                      Noch keine Analysen — starte die erste über den Tab «Analyse».
                    </td>
                  </tr>
                )}
                {list === null && (
...
function Wizard({
  ...
  const [form, setForm] = useState({
    ...
    firmenname: prefill?.firmenname || "",
    branche: prefill?.branche || "",
    ort: prefill?.ort || "",
  });
  const [aiBusy, setAiBusy] = useState(false);
  const [kandidaten, setKandidaten] = useState<Array<{
    domain: string;
    grund: string;
    checked: boolean;
  }> | null>(null);
    ...
    if (resumeId) {
      void (async () => {
        ...
        if (j?.ok) {
          setAudit(j.audit);
          setForm({
            domain: j.audit.domain,
            firmenname: j.audit.firmenname,
  ...
  const doAbbruch = async () => {
    ...
    if (
      !window.confirm(
        "Analyse wirklich abbrechen? Der Lauf wird gestoppt und kann per Re-Run neu gestartet werden.",
      )
    )
      return;
  ...
  const aiFind = async () => {
    ...
    setAiBusy(false);
    if (!j?.ok) {
      setErr(j?.error || "Wettbewerber-Ermittlung fehlgeschlagen");
      return;
    }
    setKandidaten(
      (j.kandidaten || []).map((k: any) => ({
        domain: k.domain,
        grund: k.grund,
        checked: k.empfohlen !== false,
      })),
    );
  };
  ...
  const start = async () => {
    ...
    setStartBusy(false);
    if (!j?.ok) {
      setErr(j?.error || "Start fehlgeschlagen");
      return;
    }
    if (j.bereitsLaufend)
      setErr("Für diese Domain läuft bereits eine Analyse — hier ist ihr Fortschritt.");
    setAudit(j.audit);
    setStep(4);
  ...
  const dot = (i: number) => {
    ...
    return (
      <div
        key={i}
        style={{ display: "flex", alignItems: "center", gap: 8, flex: i < 4 ? 1 : undefined }}
      >
        <div
          style={{
            width: 26,
            ...
            fontWeight: 800,
            flexShrink: 0,
            background: on ? S.grad : done ? S.greenDim : S.bg,
            color: on ? "#fff" : done ? S.green : S.dim,
            border: on || done ? "none" : `1px solid ${S.line}`,
          }}
        >
          {done ? "✓" : i}
        </div>
        {i < 4 && (
          <div
            style={{
              ...
              height: 2,
              borderRadius: 2,
              background: step > i ? S.appLight : S.line,
            }}
          />
        )}
      </div>
  ...
  return (
    <div
      className="anl-noprint"
      style={{
        position: "fixed",
        inset: 0,
        background: "rgba(22,18,23,.45)",
        zIndex: 100,
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
    ...
    >
      <div
        style={{
          ...
          maxHeight: "92vh",
          overflowY: "auto",
          boxShadow: "0 24px 70px rgba(60,0,70,.25)",
        }}
      >
        ...
        {step === 1 && (
          <>
            <div style={{ padding: "0 26px 8px" }}>
              ...
              <p style={{ fontSize: 13, color: S.mut, margin: "0 0 18px" }}>
                Domain genügt — Branche und Ort verbessern die automatische Prompt-Generierung.
              </p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                ...
                <div>
                  ...
                  <input
                    style={inp}
                    placeholder="z. B. Treuhand"
                    value={form.branche}
                    onChange={(e) => setForm({ ...form, branche: e.target.value })}
                  />
                </div>
                <div>
                  <label style={lbl}>Ort / Markt</label>
                  <input
                    style={inp}
            ...
            <div
              style={{ display: "flex", alignItems: "center", gap: 10, padding: "18px 26px 24px" }}
            >
        ...
        {step === 2 && (
          <>
            <div style={{ padding: "0 26px 8px" }}>
              ...
              <p style={{ fontSize: 13, color: S.mut, margin: "0 0 18px" }}>
                Die AI ermittelt die relevantesten Wettbewerber aus Branche, Ort und den echten
                Suchergebnissen — du kannst sie danach anpassen.
              </p>
              {!kandidaten && (
                <button
                  onClick={aiFind}
                  ...
                  style={{
                    ...
                    background: S.tint,
                    color: S.app,
                    border: `1px solid rgba(119,0,140,.3)`,
                    borderRadius: 10,
                    padding: "9px 16px",
                    fontSize: 13,
                    fontWeight: 700,
                ...
                >
                  {aiBusy
                    ? "AI analysiert Branche, SERP & AI-Antworten…"
                    : "✦ Wettbewerber mit AI ermitteln"}
                </button>
              )}
              {kandidaten && (
                <div
                ...
                >
                  {kandidaten.map((k, i) => (
                    <label
                      ...
                      style={{
                        ...
                        padding: "8px 0",
                        fontSize: 13,
                        borderBottom: i < kandidaten.length - 1 ? `1px solid ${S.line}` : "none",
                        cursor: "pointer",
                      }}
                    >
                      <input
                        ...
                        onChange={() =>
                          setKandidaten(
                            kandidaten.map((x) =>
                              x.domain === k.domain ? { ...x, checked: !x.checked } : x,
                            ),
                          )
                        }
                      />
                  ...
                  <div style={{ display: "flex", gap: 8, padding: "10px 0" }}>
                    <input
                      style={{ ...inp, flex: 1 }}
                      placeholder="weitere-domain.ch manuell ergänzen"
                      value={manuell}
                    ...
                    <button
                      style={ghostBtn}
                      onClick={() => {
                        const dm = manuell.trim().toLowerCase();
                        if (dm.includes(".")) {
                          setKandidaten([
                            ...kandidaten,
                            { domain: dm, grund: "manuell ergänzt", checked: true },
                          ]);
                          setManuell("");
                        }
                      }}
              ...
              {kandidaten && (
                <p style={{ fontSize: 11.5, color: S.dim, marginTop: 8 }}>
                  Ermittelt aus SERP-Überschneidung + AI-Kuratierung. Max. 3 gehen in den Benchmark.
                </p>
              )}
            ...
            >
              ...
              <button style={{ ...ctaBtn, marginLeft: "auto" }} onClick={() => setStep(3)}>
                {kandidaten?.some((k) => k.checked) ? "Weiter" : "Ohne Wettbewerber weiter"}
              </button>
            </div>
          </>
        )}
        ...
        {step === 3 && (
          <>
            <div style={{ padding: "0 26px 8px" }}>
              ...
              >
                Bereit — das ist im Lauf enthalten
              </h2>
              <p style={{ fontSize: 13, color: S.mut, margin: "0 0 18px" }}>
                Jede Analyse läuft im vollen Umfang — Dauer ~15 Minuten.
              </p>
              <div style={{ border: `1px solid ${S.line}`, borderRadius: 12, overflow: "hidden" }}>
                {[
                  [
                    "✦",
                    "AI-Sichtbarkeit",
                    "15 Prompts (max. 2 Brand + Kategorie- und Themen-Nischen-Suchen aus der Website) über 6 Engines (ChatGPT, Perplexity, Gemini, Claude, Grok, DeepSeek) + Makro-Mentions + AI-Suchvolumen je Prompt",
                  ],
                  [
                    "⚙",
                    "Technik & SiteHealth",
                    "robots.txt-AI-Bots, OnPage-Audit (bis 50 Seiten), Tech-Stack-Erkennung",
                  ],
                  ...
                  [
                    ...
                    `${(kandidaten || []).filter((k) => k.checked).length || "keine"} Domains in Quick-Tiefe`,
                  ],
            ...
            <div
              style={{ display: "flex", alignItems: "center", gap: 10, padding: "18px 26px 24px" }}
            >
              <button style={ghostBtn} onClick={() => setStep(2)}>
                Zurück
              </button>
              <button
                style={{ ...ctaBtn, marginLeft: "auto", opacity: startBusy ? 0.6 : 1 }}
                disabled={startBusy}
                onClick={start}
              >
          ...
          <>
            <div style={{ padding: "0 26px 8px" }}>
              ...
              <p style={{ fontSize: 13, color: S.mut, margin: "0 0 10px" }}>
                ...
                <b>du kannst dieses Fenster jederzeit schliessen</b>, der Fortschritt bleibt unter
                «Verlauf → Fortschritt» abrufbar.
              </p>
              {audit && (
                <div
                  style={{
                ...
                >
                  <span>
                    Start:{" "}
                    {new Date(audit.created_at).toLocaleTimeString("de-CH", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </span>
                  <span>
                    ...
                    {(audit as any).updated_at
                      ? new Date((audit as any).updated_at).toLocaleTimeString("de-CH", {
                          hour: "2-digit",
                          minute: "2-digit",
                          second: "2-digit",
                        })
              ...
              {STAGES.map((st2, i) => {
                ...
                if (i === 1 && d.seo)
                  zwischenbefund = `${d.seo.keywords} Keywords · ~${d.seo.etv} organische Besuche/Mt. · ${d.seo.refDomains} Ref-Domains`;
                if (i === 3 && d.prompts?.some((p: any) => p.engines)) {
                  const done2 = d.prompts.filter(
                    (p: any) => p.engines && Object.keys(p.engines).length >= 6,
                  ).length;
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      gap: 14,
                      padding: "11px 4px",
                      borderBottom: i < STAGES.length - 1 ? `1px solid ${S.line}` : "none",
                    }}
                  >
                    <div
                      style={{
                        ...
                        fontSize: 13,
                        fontWeight: 800,
                        background: done ? S.greenDim : run ? S.tint : S.bg,
                        color: done ? S.green : run ? S.app : S.dim,
                        border: done || run ? "none" : `1px dashed ${S.lineHover}`,
                      }}
                    >
                      {done ? "✓" : run ? "⟳" : i + 1}
                    </div>
              ...
              {(err || audit?.error) && audit?.status !== "fertig" && (
                <p
                  style={{
                    fontSize: 12,
                    color: audit?.status === "fehler" ? S.red : S.orange,
                    marginTop: 8,
                  }}
                >
                  {audit?.status === "fehler" ? audit?.error : err || audit?.error}
                </p>
              )}
            </div>
            <div
            ...
            >
              ...
              {audit?.status === "fehler" && (
                <button style={{ ...ctaBtn, marginLeft: "auto" }} onClick={doRetry}>
                  Erneut versuchen
                </button>
              )}
              {audit?.status === "fertig" && (
                <button style={{ ...ctaBtn, marginLeft: "auto" }} onClick={() => onDone(audit.id)}>
                  Ergebnis ansehen
                </button>
              )}
            </div>
...
function ResultView({
  ...
  const uebernehmen = async () => {
    ...
    if (!j?.ok) {
      window.alert(j?.error || "Übernahme fehlgeschlagen");
      return;
    }
  ...
  const prompts = (d.prompts || []) as Array<{
    q: string;
    vol: number | null;
  ...
  const ohneNennung = prompts.filter(
    (p) => !Object.values(p.engines || {}).some((e: any) => e?.mention),
  ).length;

  const printPdf = () => {
    // Alle Aufklapp-Panels oeffnen, damit der PDF-Export vollstaendig ist.
    document.querySelectorAll<HTMLDetailsElement>("details.anl-fold").forEach((el) => {
      el.open = true;
    });
    ...
    return (
      <div style={{ position: "relative", width: 108, height: 108, flexShrink: 0 }}>
        ...
        >
          ...
          />
          <defs>
            <linearGradient id="anlg" x1="0" y1="0" x2="1" y2="1">
  ...
  const dimCard = (label: string, n: number, art: string) => (
    ...
    >
      ...
      <div style={{ height: 5, borderRadius: 3, background: S.bg, overflow: "hidden" }}>
        <div
          style={{ width: `${n}%`, height: "100%", borderRadius: 3, background: scoreColor(n) }}
        />
      </div>
    ...
    >
      ...
      >
        ...
        <span style={{ marginLeft: "auto", color: S.app, fontWeight: 700, fontSize: 12.5 }}>
          <span className="anl-cl">Ansehen →</span>
          <span className="anl-op">Zuklappen ↑</span>
        </span>
      </summary>
  ...
  return (
    <>
      ...
      <div
        className="anl-print-head"
        style={{
          alignItems: "center",
          gap: 12,
          marginBottom: 18,
          paddingBottom: 14,
          borderBottom: `2px solid #77008C`,
        }}
      >
        <div
          style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: S.mut }}>
            {audit.firmenname} · {audit.domain} · {fmtDate(audit.created_at)} · ezyone.ch
          </div>
        </div>
      </div>
      ...
      >
        <div>
          <span
            style={{ fontSize: 12, color: S.app, fontWeight: 600, cursor: "pointer" }}
            onClick={onBack}
          >
            ‹ Zurück zum Verlauf
          </span>
          ...
          <div style={{ fontSize: 13, color: S.mut, marginTop: 4 }}>
            {audit.firmenname} • Analyse vom {fmtDate(audit.created_at)}
          </div>
        </div>
        <div
          style={{
        ...
        >
          <button style={ghostBtn} onClick={onRerun}>
            Re-Run
          </button>
          {isOrgAdmin &&
            (kundeId ? (
              <a
                href={`/admin?client=${kundeId}`}
                style={{
                  ...ghostBtn,
                  textDecoration: "none",
                  color: S.app,
                  borderColor: `${S.app}55`,
                  display: "inline-block",
                }}
              >
                Kunde im Admin öffnen
              </a>
            ) : (
              <button
                style={{
        ...
        <div style={{ display: "flex", gap: 22, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1, minWidth: 260 }}>
            <div style={{ fontSize: 13.5, lineHeight: 1.6 }}>
              ...
              {blocked.length ? (
                <span style={{ color: S.red, fontWeight: 700 }}>
                  AI-Crawler geblockt ({blocked.slice(0, 3).join(", ")})
                </span>
              ) : (
            ...
            <div
              style={{
                background: S.tint,
                border: "1px solid rgba(119,0,140,.18)",
                borderRadius: 12,
                padding: "12px 16px",
                fontSize: 13,
                marginTop: 16,
            ...
            >
              <b style={{ color: S.appLight, fontSize: 15 }}>
                Verpasstes AI-Suchvolumen: {fmtVol(missed)} Anfragen/Monat
              </b>
              <br />
              Bei {ohneNennung} von {prompts.length} Prompts wird die Marke in keiner Engine genannt
              — dieses Volumen beantworten aktuell Wettbewerber und Verzeichnisse.
            </div>
          </div>
        </div>
        <div
      ...
      {d.aivisKunde?.name && (
        ...
        >
          ...
          <div style={{ fontSize: 13, lineHeight: 1.6 }}>
            <b>{d.aivisKunde.name} ist bereits EzyAI-Kunde.</b> Das dortige Monitoring misst den
            kuratierten Prompt-Korpus <em>inklusive Brand-Prompts</em> über 30 Tage und alle Modelle
            — dadurch fallen Score, Erwähnungen und Citations höher aus. Diese Analyse misst bewusst{" "}
            {`${(d.prompts || []).filter((p: any) => !p.brand).length}`} neutrale
            Alternativen-Suchen: Sie zeigt, wie oft die Marke empfohlen wird, wenn{" "}
            <em>niemand nach ihr fragt</em>. Beide Werte sind korrekt — sie beantworten
      ...
      <div style={{ ...card, marginTop: 16 }}>
        ...
        <p style={{ fontSize: 12.5, color: S.mut, margin: "0 0 14px" }}>
          Jeder Prompt mit AI-Suchvolumen und Nennung je Engine
          {d.mentionsMakro?.total !== undefined
            ? ` · Makro-Erwähnungen über alle Plattformen: ${d.mentionsMakro.total}`
            : ""}
          .
        </p>
        <div style={{ overflowX: "auto" }}>
          ...
          >
            <thead>
              <tr>
                ...
                <th style={{ ...th, textAlign: "right" }}>AI-Vol./Mt.</th>
                {ENGINE_COLS.map((e) => (
            ...
            <tbody>
              {prompts.map((p) => {
                const cited = [
                  ...new Set(
                    Object.values(p.engines || {}).flatMap((e: any) =>
                      e?.mention ? [] : e?.cited || [],
                    ),
                  ),
                ]
                  .filter((c: any) => !String(c).includes(audit.domain))
                  .slice(0, 3);
                return (
                  <tr key={p.q}>
                    <td style={td}>
                      ...
                      {(p as any).brand && (
                        <span
                          style={{ ...pillStyle(S.tint, S.app), marginLeft: 8, fontSize: 10.5 }}
                        >
                          Brand
                        </span>
                      )}
                    ...
                    <td style={{ ...td, textAlign: "right" }}>
                      {p.vol === null || p.vol === undefined
                        ? "—"
                        : Math.round(p.vol).toLocaleString("de-CH")}
                    </td>
                    {ENGINE_COLS.map((e) => {
                      const r = p.engines?.[e];
                      return (
                        <td
                          ...
                          style={{
                            ...
                            textAlign: "center",
                            fontWeight: 800,
                            color: !r || !r.ok ? S.dim : r.mention ? S.green : S.red,
                          }}
                        >
                          {!r || !r.ok ? "·" : r.mention ? "✓" : "–"}
                        </td>
                    ...
                    <td style={{ ...td, color: S.mut, fontSize: 12.5 }}>
      ...
      {Array.isArray(d.technik?.botDetails) &&
        d.technik.botDetails.length > 0 &&
        (() => {
          const bots = d.technik.botDetails as Array<{
            name: string;
            owner: string;
            status: string;
          }>;
          ...
          return fold(
            "🤖",
            blockiert ? S.redDim : S.tint,
            blockiert ? S.red : S.app,
            ...
            <div>
              <p style={{ fontSize: 12.5, color: S.mut, margin: "8px 0 10px" }}>
                Quelle: https://{audit.domain}/robots.txt — «erlaubt» = eigener User-agent-Block,
                nicht per{" "}
                <code style={{ background: S.bg, borderRadius: 4, padding: "1px 5px" }}>
                  Disallow: /
                </code>{" "}
                gesperrt. «Nicht spezifiziert» = keine eigene Regel, es gelten die *-Regeln der
                robots.txt.
              </p>
              <div style={{ overflowX: "auto" }}>
        ...
        <div style={{ ...card, marginTop: 16, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12.5, color: S.mut }}>
              Ohne robots.txt gelten keine Bot-Regeln — alle Crawler dürfen lesen, aber die
              Steuerungsmöglichkeit fehlt.
            </div>
          </div>
        </div>
      )}
      ...
      {fold(
        ...
        S.orangeDim,
        S.orange,
        `${issues.length} Probleme gefunden`,
        "— aus dem SiteHealth-Audit",
        <div>
          {issues.map((i) => (
            <div
              key={i.id}
            ...
            >
              ...
              <span style={{ marginLeft: "auto", color: S.dim, fontSize: 12, flexShrink: 0 }}>
                {i.pages?.length ? `${i.pages.length} Seiten` : i.detail}
              </span>
            </div>
          ))}
          {issues.length === 0 && (
            <div style={{ color: S.dim, fontSize: 12.5, padding: "8px 0" }}>
          ...
          >
            <thead>
              <tr>
                ...
                <th style={{ ...th, textAlign: "right" }}>Probleme</th>
                ...
                <tr key={p.path}>
                  ...
                  <td
                    style={{
                      ...td,
                      textAlign: "right",
                      color: p.status >= 400 || p.status === 0 ? S.red : S.txt,
                      fontWeight: p.status >= 400 || p.status === 0 ? 700 : 400,
                    }}
                  >
                    {p.status || "—"}
                  </td>
      ...
      >
        <div style={card}>
          ...
          <p style={{ fontSize: 12.5, color: S.mut, margin: "0 0 8px" }}>
            Automatisch aus den Befunden abgeleitet, nach Wirkung/Aufwand priorisiert.
          </p>
          {(d.massnahmen || []).map((m: any, i: number) => (
            <div
              ...
              style={{
                ...
                gap: 12,
                padding: "11px 0",
                borderBottom: i < (d.massnahmen || []).length - 1 ? `1px solid ${S.line}` : "none",
                alignItems: "flex-start",
              }}
            >
              <span
        ...
        <div style={card}>
          ...
          <p style={{ fontSize: 12.5, color: S.mut, margin: "0 0 8px" }}>
            Wettbewerber via AI ermittelt und in Quick-Tiefe mitanalysiert.
          </p>
          {[
            { ...(bench.eigen || {}), domain: audit.domain, eigen: true },
            ...(bench.wettbewerber || []),
          ].map((b: any) => (
            <div
              key={b.domain}
              ...
              <div style={{ height: 9, borderRadius: 5, background: S.bg, overflow: "hidden" }}>
                <div
                  style={{
                    ...
                    height: "100%",
                    borderRadius: 5,
                    background: b.eigen ? S.grad : S.lineHover,
                  }}
                />
              </div>
              <span
          ...
          {(bench.wettbewerber || []).length > 0 &&
            (() => {
              const top = [...bench.wettbewerber].sort(
                (a: any, b: any) => (b.mentions || 0) - (a.mentions || 0),
              )[0];
              return top?.mentions > (bench.eigen?.mentions || 0) ? (
                ...
                >
                  <b style={{ color: S.appLight }}>Pitch-Argument:</b> {top.domain} wird{" "}
                  {top.mentions}× in AI-Antworten zitiert — {audit.domain}{" "}
                  {bench.eigen?.mentions || 0}×.
                </div>
              ) : null;
            })()}
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            <Chip
              ...
              label={
                anb.cms
                  ? `CMS: ${anb.cms}${anb.builder ? ` + ${anb.builder}` : ""}`
                  : "CMS nicht erkannt"
              }
              warnIfMissing
            />
            <Chip
              ok={!!anb.ga4}
              label={
                anb.ga4 ? `GA4${anb.gaId ? ` (${anb.gaId})` : ""} erkannt` : "kein Analytics-Tag"
              }
            />
            <Chip
              ok={!!anb.gtm}
              label={
                anb.gtm
                  ? `Tag Manager${anb.gtmId ? ` (${anb.gtmId})` : ""} erkannt`
                  : "kein Tag Manager"
              }
              warnIfMissing
            />
            <Chip
              ok={!!anb.consent}
              label={anb.consent ? `Consent-Tool: ${anb.consent}` : "kein Consent-Tool erkannt"}
              warnIfMissing
            />
            {anb.adsTag ? (
              ...
              <Chip
                ok={false}
                label="Ads-Tag nicht im HTML sichtbar (evtl. im GTM-Container)"
                warnIfMissing
              />
            ) : (
          ...
          {ent.wikidata && (
            <>
              ...
              <div style={{ fontSize: 12.5, color: S.mut, lineHeight: 1.7 }}>
                Wikidata:{" "}
                {ent.wikidata.found ? (
                  <b style={{ color: S.green }}>{ent.wikidata.id} vorhanden</b>
                ) : (
                  <b style={{ color: S.orange }}>kein eindeutiges Item</b>
                )}
                {" · "}Organization-Schema:{" "}
                {ent.orgSchema ? (
                  <b style={{ color: S.green }}>vorhanden</b>
                ) : (
                  <b style={{ color: S.orange }}>fehlt</b>
                )}
                {" · "}Brand-SERP:{" "}
                {ent.brandPos === 1 ? (
                  <b style={{ color: S.green }}>#1 auf den eigenen Namen</b>
                ) : ent.brandPos ? (
                  <b style={{ color: S.orange }}>#{ent.brandPos}</b>
                ) : (
                  <b style={{ color: S.red }}>nicht in den Top 10</b>
                )}
              </div>
            </>
          )}
      ...
      <div style={{ ...card, marginTop: 16 }}>
        ...
        <p style={{ fontSize: 12.5, color: S.mut, margin: "0 0 10px" }}>
          Messdatum: {fmtDate(audit.created_at)} · Alle Abfragen aus der Schweiz (de-CH),
          Momentaufnahme eines einzelnen Laufs.
        </p>
        <div style={{ overflowX: "auto" }}>
          <table style={{ borderCollapse: "collapse", width: "100%" }}>
            <thead>
            ...
            <tbody>
              {[
                [
                  "Prompts × Engines",
                  "Live-Antworten: DataForSEO-Scraper (ChatGPT, Perplexity, Gemini, Claude) + Direkt-APIs (Grok, DeepSeek)",
                  "gemessen",
                ],
                [
                  "AI-Suchvolumen je Prompt",
                  "DataForSEO ai_keyword_data (Monatswerte)",
                  "gemessen",
                ],
                ["Makro-Erwähnungen", "DataForSEO llm_mentions (Drittkorpus)", "gemessen"],
                [
                  "SiteHealth, Probleme, Seiten, AI-Crawler",
                  "Eigener Live-Crawl (robots.txt, HTML, Sitemap)",
                  "gemessen",
                ],
                [
                  "SEO-Fundament (Keywords, Traffic, Backlinks)",
                  "DataForSEO Labs",
                  "Schätzwerte (Modell)",
                ],
                [
                  "Entität (Wikidata, Brand-SERP, Schema)",
                  "Wikidata-API + Google-SERP + HTML",
                  "gemessen",
                ],
                [
                  "Wettbewerber-Benchmark (Quick-Score)",
                  "Labs-Schätzung + robots.txt + Engine-Nennungen",
                  "berechnet",
                ],
                [
                  "AI-Readiness-Score & Dimensionen",
                  "Gewichtung 40 % AI · 25 % Technik · 20 % Entität · 15 % SEO",
                  "berechnet",
                ],
                ...
                <tr key={b as string}>
                  ...
                  <td style={td}>
                    <span
                      style={pillStyle(
                        a === "gemessen" ? S.greenDim : a === "AI-generiert" ? S.tint : S.orangeDim,
                        a === "gemessen" ? S.green : a === "AI-generiert" ? S.app : S.orange,
                      )}
                    >
                      {a}
                    </span>
      ...
      <p className="anl-noprint" style={{ fontSize: 11.5, color: S.dim, marginTop: 12 }}>
        Der PDF-Export erzeugt den vollständigen Report im Ezy-One-CI (Druckdialog → «Als PDF
        speichern»); alle Aufklapp-Panels werden dafür automatisch geöffnet — inklusive Methodik &
        Datenquellen.
      </p>
    </>
  );
