...
peak as (
  select content_item_id, max(clicks_28) as peak_clicks_28,
    min(pos_28) filter (where pos_28 is not null) as peak_position
  from rolling group by content_item_id
),
cannib as (
  ...
  group by client_id, lower(primary_keyword) having count(*) > 1
)
select
  ...
  c.clicks_28, c.impr_28, pk.peak_position, pk.peak_clicks_28,
  case
