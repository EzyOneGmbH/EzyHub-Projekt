-- ============================================================================
-- Unsichtbare Alt-Artikel als handlungsbeduerftig flaggen.
-- Ein publizierter Blogartikel, der nach 90+ Tagen NULL Impressionen hat, ist
-- kein "Zu wenig Daten"-Fall (dort blieb er bisher fuer immer haengen), sondern
-- ein Index-/Verlinkungsproblem -> recommendation 'tech_fix'.
-- Nur die View wird neu erstellt; RPC/RLS bleiben unveraendert.
-- ============================================================================
...
select
  ...
  case
    ...
    -- NEU: 90+ Tage publiziert und keinerlei Impressionen = in Google unsichtbar
    -- -> Technik pruefen (Indexierung/interne Verlinkung), nicht ewig beobachten.
    when (current_date - ci.published_at) >= 90
         and coalesce(c.impr_28,0) = 0 then 'tech_fix'
